...
- Markdown 使用 `MarkdownDocumentReader`，PDF 使用 `PagePdfDocumentReader` 按页读取，其他文本和 Office 文档使用 `TikaDocumentReader`。
...
Markdown reader 会将标题作为 reader 元数据处理，正文切片使用其正文输出。PDF/Office reader 返回的已知页码元数据会写入父、子文档；图片抽取按单页处理并标记为第 1 页。表格单元格中的反斜杠、竖线和换行会被转义/规范化，避免破坏 Markdown 表格结构。
